class HealthMaintenanceOrganizationsController < ApplicationController
  ...
  def create
    ...
    if @hmo.save
      ...
      redirect_to health_maintenance_organizations_path
    else
  ...
  def update
    ...
    if @hmo.update(hmo_params)
      flash[:success] = "Category successfully created."
      redirect_to edit_health_maintenance_organization_path(@hmo.id)
    else
      flash[:success] = "Category successfully created."
      redirect_to edit_health_maintenance_organization_path(@hmo.id)
    end
